...
import sanchana.bagShop.asset.commonAsset.model.Enum.*;
  ...
  public String addEmployee(@Valid @ModelAttribute Employee employee, BindingResult result, Model model
    ...
    if ( employee.getId() == null ) {
      ...
      if ( lastEmployee.getCode() == null ) {
        employee.setCode("SBSE" + makeAutoGenerateNumberService.numberAutoGen(null).toString());
      } else {
        employee.setCode("SBSE" + makeAutoGenerateNumberService.numberAutoGen(lastEmployee.getCode().substring(4)).toString());
      }
    }
